...
export const sassConvertMissingCommandMessage: string =
	"Please install the sass command line tools from http://sass-lang.com/install if you want to use Sass Formatter extension.";
export const sassConvertInvalidSassPathMessage: string =
	"The sassPath setting is not valid.";
export const sassConvertErrorMessage: string =
	"There was an error formatting your file. See Output panel for more details.";
...
export class SassConvertService implements SassConvert {
	...
	private _checkSassConvert(): void {
		try {
			let sassConvertVersion: string = execSync(
				`${this.sassConvertCommand} --version`
			)
				.toString("utf8")
				.trim();
	...
	public setSassConvertCommand(): any {
		let configurationSassPath = workspace
